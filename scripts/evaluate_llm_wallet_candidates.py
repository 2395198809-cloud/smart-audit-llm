...
REPORT_PATH = Path("reports/llm_wallet_patch_candidates_report.md")
SLITHER_REPORT_DIR = Path("reports/candidate_wallet_slither")
...
def has_tx_origin(slither_json_path: Path) -> bool:
    ...
    return any(finding.get("check") == "tx-origin" for finding in findings)
...
def main():
    candidates = sorted(CANDIDATES_DIR.glob("LLMFixedWallet_candidate_*.sol"))

    if not candidates:
        raise SystemExit("No wallet candidates found. Run python3 scripts/generate_llm_wallet_patch.py first.")
    ...
    try:
        for candidate in candidates:
            ...
            code, out, err = run_command(
                ["python3", "scripts/validate_llm_wallet_patch.py", str(candidate)]
            )
            structure_ok = code == 0
            ...
            if structure_ok:
                ...
                if build_ok:
                    code, out, err = run_command(
                        ["forge", "test", "--match-contract", "LLMFixedWalletTest", "-vv"]
                    )
                    test_ok = code == 0
                ...
                if test_ok:
                    ...
                    slither_ok = code in (0, 255) and not has_tx_origin(slither_output)

                    detail_lines.append("### Slither重扫")
                    detail_lines.append("```text")
                    detail_lines.append((out + err).strip() or "(no output)")
                    detail_lines.append("```")
                    detail_lines.append("")
    ...
    lines = []
    lines.append("# LLM tx.origin补丁候选方案评估报告")
    lines.append("")
    lines.append("| 候选方案 | 结构校验 | 编译 | 测试 | Slither重扫 | 是否推荐 |")
    lines.append("|---|---|---|---|---|---|")
    ...
    lines.append("")
    lines.append("说明：本脚本只评估tx.origin候选补丁，不执行最终应用。通过全部检查的候选方案仍需人工审批后才能写入正式源码。")
    lines.append("")
    lines.extend(detail_lines)

    REPORT_PATH.write_text("\n".join(lines), encoding="utf-8")

    print(f"\nWallet candidate report written to {REPORT_PATH}")
    print("Evaluation finished. src/LLMFixedWallet.sol has been restored to its original content.")
