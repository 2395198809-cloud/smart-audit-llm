...
SUMMARY_MD = OUT / "system_v2_repeat_60_summary_with_postcheck.md"
ROUND_CSV = OUT / "system_v2_repeat_60_accuracy_by_round_with_postcheck.csv"
VAR_CSV = OUT / "system_v2_repeat_60_accuracy_variance_with_postcheck.csv"
TIME_CSV = OUT / "system_v2_repeat_60_runtime_variance_with_postcheck.csv"
DETAIL_CSV = OUT / "system_v2_repeat_60_sample_stage_detail_with_postcheck.csv"
...
def raw_report_detected(artifact_dir):
...
def has_authorization_guard(source_text):
    s = re.sub(r"\s+", " ", source_text)

    guards = [
        # ERC20 / Vault allowance 正确方向
        r"msg\.sender\s*!=\s*from.*allowance\s*\[\s*from\s*\]\s*\[\s*msg\.sender\s*\]",
        r"msg\.sender\s*!=\s*owner.*allowance\s*\[\s*owner\s*\]\s*\[\s*msg\.sender\s*\]",
        r"allowance\s*\[\s*from\s*\]\s*\[\s*msg\.sender\s*\]\s*>=\s*amount",
        r"allowance\s*\[\s*owner\s*\]\s*\[\s*msg\.sender\s*\]\s*>=\s*amount",

        # ERC721 正确 approval / operator
        r"msg\.sender\s*==\s*from",
        r"getApproved\s*\[\s*tokenId\s*\]\s*==\s*msg\.sender",
        r"isApprovedForAll\s*\[\s*from\s*\]\s*\[\s*msg\.sender\s*\]",

        # ERC1155 正确 operator
        r"isApprovedForAll\s*\[\s*from\s*\]\s*\[\s*msg\.sender\s*\]",
    ]

    return any(re.search(g, s, re.I) for g in guards)

def postchecked_detected(sample, artifact_dir):
    """
    后置校验逻辑：
    1. 原始报告没报目标漏洞，则后置也不报。
    2. 原始报告报了目标漏洞，但源码存在明确正确授权保护，则后置校验抑制该告警。
    3. 否则保留告警。
    """
    raw = raw_report_detected(artifact_dir)
    if raw == 0:
        return 0

    sol = ROOT / "benchmarks/gptscan" / f"{sample}.sol"
    src = read_text(sol)

    if has_authorization_guard(src):
        return 0
...
for r in runtime_rows:
    if r.get("round") and r.get("sample") and r.get("stage"):
...
for rd in [1, 2, 3]:
    ...
    for sample in vuln_samples:
        ...
        raw = raw_report_detected(artifact)
        post = postchecked_detected(sample, artifact)
        ...
        details.append({
            ...
            "sample": sample,
            "type": "vulnerable",
            "raw_report_detected": raw,
            "postchecked_report_detected": post,
            "compile_ok": ev["compile_ok"],
            "attack_ok": ev["attack_ok"],
            "repair_ok": ev["repair_ok"],
            "raw_safe_fp": "",
    ...
    for sample in safe_samples:
        ...
        gen = sm.get("auto_llm_generate_v2", {})
        artifact = gen.get("artifact_dir") or str(OUT / f"round_{rd}/artifacts/{sample}_qwen3_coder_30b")
        eval_md = str(OUT / f"round_{rd}/evaluations/{sample}_evaluation.md")

        raw_fp = raw_report_detected(artifact)
        post_fp = postchecked_detected(sample, artifact)

        raw_safe_fp += raw_fp
        post_safe_fp += post_fp
        ...
        details.append({
            ...
            "type": "safe",
            "raw_report_detected": "",
            "postchecked_report_detected": "",
            "compile_ok": "",
            "attack_ok": "",
            "repair_ok": "",
            "raw_safe_fp": raw_fp,
            "postchecked_safe_fp": post_fp,
            "artifact_dir": artifact,
            "evaluation_md": eval_md,
        })
    ...
    round_stats.append({
        "round": rd,
        "raw_report_ok": raw_report_ok,
        "post_report_ok": post_report_ok,
        "compile_ok": compile_ok,
        "attack_ok": attack_ok,
        "repair_ok": repair_ok,
        "raw_safe_fp": raw_safe_fp,
        "post_safe_fp": post_safe_fp,
...
with ROUND_CSV.open("w", newline="", encoding="utf-8") as f:
    w = csv.DictWriter(f, fieldnames=[
        "round",
        "raw_report_rate","raw_report_ok","report_n",
        "postchecked_report_rate","postchecked_report_ok",
        "compile_rate","compile_ok","compile_n",
        "attack_rate","attack_ok","attack_n",
        "repair_rate","repair_ok","repair_n",
        "raw_safe_fp_rate","raw_safe_fp","safe_n",
        "postchecked_safe_fp_rate","postchecked_safe_fp",
    ])
    w.writeheader()
    for s in round_stats:
        ...
        w.writerow({
            "round": s["round"],
            "raw_report_rate": f"{pct(s['raw_report_ok'], n):.2f}",
            "raw_report_ok": s["raw_report_ok"],
            "report_n": n,
            "postchecked_report_rate": f"{pct(s['post_report_ok'], n):.2f}",
            "postchecked_report_ok": s["post_report_ok"],
            "compile_rate": f"{pct(s['compile_ok'], n):.2f}",
            "compile_ok": s["compile_ok"],
            "compile_n": n,
            "attack_rate": f"{pct(s['attack_ok'], n):.2f}",
            "attack_ok": s["attack_ok"],
            "attack_n": n,
            "repair_rate": f"{pct(s['repair_ok'], n):.2f}",
            "repair_ok": s["repair_ok"],
            "repair_n": n,
            "raw_safe_fp_rate": f"{pct(s['raw_safe_fp'], sn):.2f}",
            "raw_safe_fp": s["raw_safe_fp"],
            "safe_n": sn,
            "postchecked_safe_fp_rate": f"{pct(s['post_safe_fp'], sn):.2f}",
            "postchecked_safe_fp": s["post_safe_fp"],
        })

metrics = {
    "原始审计报告识别率": [pct(s["raw_report_ok"], s["vuln_n"]) for s in round_stats],
    "后置校验审计报告识别率": [pct(s["post_report_ok"], s["vuln_n"]) for s in round_stats],
    ...
    "原始安全样本误报率": [pct(s["raw_safe_fp"], s["safe_n"]) for s in round_stats],
    "后置校验安全样本误报率": [pct(s["post_safe_fp"], s["safe_n"]) for s in round_stats],
}
...
md = []
md.append("# 本文系统60样本三轮重复实验汇总（含后置校验）\n")
md.append(f"- 样本数：{len(manifest)}")
md.append(f"- 漏洞样本数：{len(vuln_samples)}")
md.append(f"- 安全样本数：{len(safe_samples)}")
md.append("- 说明：后置校验用于抑制已存在明确授权检查的安全样本误报。\n")

md.append("## 各轮准确率\n")
md.append("| 轮次 | 原始审计报告识别率 | 后置校验审计报告识别率 | 补丁编译通过率 | 攻击PoC成功率 | 修复验证通过率 | 原始安全样本误报率 | 后置校验安全样本误报率 |")
md.append("|---:|---:|---:|---:|---:|---:|---:|---:|")
...
for s in round_stats:
    ...
    md.append(
        f"| {s['round']} | "
        f"{s['raw_report_ok']}/{n} = {pct(s['raw_report_ok'], n):.1f}% | "
        f"{s['post_report_ok']}/{n} = {pct(s['post_report_ok'], n):.1f}% | "
        f"{s['compile_ok']}/{n} = {pct(s['compile_ok'], n):.1f}% | "
        f"{s['attack_ok']}/{n} = {pct(s['attack_ok'], n):.1f}% | "
        f"{s['repair_ok']}/{n} = {pct(s['repair_ok'], n):.1f}% | "
        f"{s['raw_safe_fp']}/{sn} = {pct(s['raw_safe_fp'], sn):.1f}% | "
        f"{s['post_safe_fp']}/{sn} = {pct(s['post_safe_fp'], sn):.1f}% |"
    )
